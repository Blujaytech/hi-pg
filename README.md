...
- **Phase 7** — Receipts are snapshot-based and sequentially numbered. Aadhaar/photo/KYC documents use private S3-compatible storage with time-limited signed download URLs when `S3_ENABLED=true`; local development retains an explicit 501 stub when storage is disabled.
...
- **Phase 12 / flexible-booking expansion** — Razorpay Checkout is wired into booking and fee payment screens, with server-side signature/capture verification, idempotent webhooks, automatic refunds for expired holds, Razorpay Route owner settlements, and customer-authorized monthly AutoPay. Calls return `501` only when Razorpay credentials are absent; see ADR-0027.
...
All 16 original phases plus the flexible-booking/payment expansion are implemented. Before a live launch, provision infrastructure and credentials, register the Razorpay webhook and Route linked accounts, enable the private S3 document gateway, and connect a push/email delivery provider. The local backend and Flutter verification commands are run as part of this expansion; live money movement still requires a Razorpay test/live account.
