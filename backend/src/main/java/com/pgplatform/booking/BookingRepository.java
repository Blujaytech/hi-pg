...
public interface BookingRepository extends JpaRepository<Booking, UUID> {
    ...
    @Query("select count(b) from Booking b where b.student.user.id = :userId and b.status in ('PAYMENT_PENDING', 'CONFIRMED', 'CHECKED_IN') and b.deletedAt is null")
    long countActiveConfirmedForUser(@Param("userId") UUID userId);

    @Query(value = """
            select count(*) from bookings b
            where b.bed_id = :bedId
              and b.status in ('PAYMENT_PENDING', 'CONFIRMED', 'CHECKED_IN')
    ...
    @Query("select b from Booking b where b.bed.room.id = :roomId and b.deletedAt is null " +
           "and b.status in ('PAYMENT_PENDING', 'CONFIRMED', 'CHECKED_IN', 'COMPLETED') " +
