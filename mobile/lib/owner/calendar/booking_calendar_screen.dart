...
class _BookingCalendarScreenState extends State<BookingCalendarScreen> {
  ...
  void _load() {
    _future = _repository.list(
        widget.roomId, _from, DateTime(_from.year, _from.month + 2, 1));
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar:
          AppBar(title: Text('Room ${widget.room?.roomNumber ?? ''} calendar')),
      body: Column(children: [
        Padding(
          ...
          child: Column(children: [
            Row(children: [
              ...
              Expanded(
                  child: Text(DateFormat('MMMM yyyy').format(_from),
        ...
        Expanded(
            child: FutureBuilder<List<BookingCalendarEntry>>(
          future: _future,
          builder: (context, snapshot) {
            ...
            if (entries.isEmpty) {
              return const AppEmptyView(
                  ...
                  message: 'All beds are free for the selected dates.');
            }
            return ListView.separated(
              ...
              itemBuilder: (context, index) {
                ...
                final end = entry.endDate == null
                    ? 'Open-ended'
                    : '${DateFormat('d MMM yyyy').format(entry.endDate!)}${entry.checkOutTime == null ? '' : ' at ${entry.checkOutTime!.substring(0, 5)}'}';
                return Card(
                    child: ListTile(
                  leading: Container(
                      width: 6,
                  ...
                  title: Text('${entry.bedLabel} · ${entry.customerName}',
                      style: const TextStyle(fontWeight: FontWeight.w700)),
                  subtitle: Text(
                      '${DateFormat('d MMM yyyy').format(entry.startDate)} → $end\n${entry.bookingType == 'DAY_WISE' ? 'Day-wise' : 'Monthly'} · ${entry.status.replaceAll('_', ' ')}'),
                  isThreeLine: true,
                ));
              },
            );
