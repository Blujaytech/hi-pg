...
Metadata-only table; document bytes live in the configured private S3-compatible bucket. `DocumentService.upload` stores the bytes before inserting metadata, so a failed object write cannot leave a row with a broken key. With `S3_ENABLED=false`, the fail-closed stub returns 501 before persistence.
