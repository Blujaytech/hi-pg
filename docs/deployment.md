...
**Render specifically**: `render.yaml` at the repo root is a Blueprint -- import it from
[the Render dashboard](https://dashboard.render.com/blueprints) ("New Blueprint Instance",
point at this repo) and it provisions the backend web service plus a managed Postgres database
together, wired to each other. You still fill in `SPRING_DATASOURCE_URL` (Render can't hand you
a JDBC-formatted value automatically -- build it from the database's Host/Port/Database as
`jdbc:postgresql://<host>:<port>/<database>`) and `CORS_ALLOWED_ORIGINS` by hand in the
dashboard after import; `JWT_SECRET` is auto-generated by the blueprint.
