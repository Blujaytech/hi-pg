...
Search response is `PagedResponse<PgSearchResultResponse>` -- a project-wide pagination envelope (`{content, page, size, totalElements, totalPages}`, see `docs/decisions.md`), not Spring's raw `Page`. `PgSearchResultResponse`: `{id, name, city, address, description, photoUrl, genderPreference, latitude, longitude, availableBeds, minRentPerBed, maxRentPerBed, offersMonthly, offersDayWise, minDayWiseRate}` -- rent range is derived from that PG's `Room.rentPerBed` values, not a stored field on the PG itself. `offersMonthly` is true when any room is `MONTHLY` or `MIXED`, `offersDayWise` when any room is `DAY_WISE` or `MIXED`; `minDayWiseRate` is the lowest `dayWiseRate` among day-wise-capable rooms (`null` when the PG has none). Clients use these to label stay types on search cards and to filter by stay type.
...
Each room in `/public/pgs/{pgId}` also carries `beds: [{id, label, available}]` -- every non-deleted bed in the room, `available` meaning it is in `availableBedOptions` for the requested dates and booking type. It exists so clients can draw the whole room and grey out taken beds; it deliberately does not say *why* a bed is unavailable (booked vs. maintenance) or who is in it. Older clients can ignore it.
